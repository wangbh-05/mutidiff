...
**一句话结论**：
> DPP 将 Diffusion Policy 的**动作多样性显著提升 21%**，同时**对成功率无统计可检测的影响**
> （Δ=+0.8pp, 95% CI [−2.0pp, +3.6pp]）。在实际部署中，从 K=8 个候选动作中
> 选择最优者，期望成功率至少与不加 guidance 持平，但拥有更多选择。

**为什么这个结论有意义**：
- Push-T 任务中，同一个观测下可能有多种推法（从左推、从右推、从上方推）
- 标准 DP 只输出一种。DPP 在保持质量的前提下提供了 K=8 个不同的候选方案
- 对于需要躲避障碍物、处理多接触点的真实任务，有备选方案是实际部署的必要条件
...
修正的 Trajectory Diversity 评估（K 个并行环境各执行 action[k]）验证了
guidance 产生了真正不同的轨迹。action[0..7] 被各自执行后，agent 路径
在环境中开始分叉。对于 Push-T 任务，block 终点多样性受限于目标区域大小
（20×20），不是衡量多样性的主要指标。更复杂的任务（如 Kitchen 多子任务）
中轨迹分叉的效果会更显著。
